...
# Kolmogorov-Smirnov Tests distributions considered under the null hypothesis are
# continuous distributions but are otherwise unrestricted.  non-parametric and
# distribution free

# No lsq and no ncor
all_species_no_Lsq_Ncor <- data.frame(c(na.omit(all_species[, 1]), na.omit(all_species[,
    2]), na.omit(all_species[, 3])))
...
# no Ngib
stacked_no_Ngib <- stack_all_species[which(stack_all_species$Species != "Ngib_data" &
    stack_all_species$Species != "Lsq_data"), 1]

AFGP_Pos_stacked_no_Ngib <- data.frame(AFGP_Pos = stacked_no_Ngib)

## Test for normality shapiro.test(AFGP_Pos_data$AFGP_Pos) # reject null (not
## normal distribution) qqnorm(AFGP_Pos_data$AFGP_Pos) # does not look normal
## (values do not fall on a straight line)

# shapiro.test(AFGP_Neg_data$AFGP_Neg) # reject null (not normal distribution)
# qqnorm(AFGP_Neg_data$AFGP_Neg) # does not look normal (values do not fall on a
# straight line)

## Plot CDFs An empirical cumulative distribution function (CDF) is a
## non-parametric estimator of the underlying CDF of a random variable.  It
## assigns a probability of 1/n to each datum, orders the data from smallest to
## largest in value, and calculates the sum of the assigned probabilities up to
## and including each datum.  The result is a step function that increases by 1/n
## at each datum.

AFGP_Pos.ecdf <- ecdf(AFGP_Pos_data$AFGP_Pos)
AFGP_Neg.ecdf <- ecdf(AFGP_Neg_data$AFGP_Neg)

# ## Plot individual CDF for AFGP Pos vs Neg plot(AFGP_Pos.ecdf, xlab =
# parse(text=paste('Temperature (C','^o',')')), ylab = '', main = 'CDF of AFGP
# Positive Species Temperature Distribution' ) mtext(text =
# expression(hat(F)[n](x)), side = 2, line = 2.5) plot(AFGP_Neg.ecdf, xlab =
# parse(text=paste('Temperature (C','^o',')')), ylab = '', main = 'CDF of AFGP
# Negative Species Temperature Distribution' ) mtext(text =
# expression(hat(F)[n](x)), side = 2, line = 2.5)
# ############################################################################################################
# Kolmogorov-Smirnov Tests The KS-test uses the maximum vertical deviation
# between the two curves as the statistic D.
...
KS_Test_AFGP_neg_vs_pos_NO_Ngib <- ks.test(as.numeric(AFGP_Pos_stacked_no_Ngib$AFGP_Pos),
    as.numeric(AFGP_Neg_data$AFGP_Neg))
...
stat_ecdf(na.rm = TRUE) + scale_color_manual(values = c(rgb(224, 29, 27, maxColorValue = 255),
    rgb(48, 115, 175, maxColorValue = 255))) + xlim(-2, 2) + ylab("Cumulative Probablility") +
    xlab(parse(text = paste("Temperature (C", "^o", ")"))) + geom_vline(xintercept = 1.49,
    color = "red", linetype = "dashed", alpha = 0.5) + geom_vline(xintercept = -1.13,
    color = "blue", linetype = "dashed", alpha = 0.5) + scale_y_continuous(expand = c(0,
    0)) + theme_bw()
